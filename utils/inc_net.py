...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        if args.get("model_name") == "sdlora":
            ...
            rank = args.get("lora_rank", 10)
            filepath = args.get("filepath", "./")
            increment = args.get("increment", 0)
            init_classes = args.get("init_cls", 0)
            model = LoRA_ViT_timm(
                vit_model=base_model.eval(),
                r=rank,
                num_classes=init_classes or 10,
                increment=increment,
                filepath=filepath,
            )
            model.out_dim = 768
...
class IncrementalNet(BaseNet):
    ...
    def save_fc(self, directory, task_id):
        if directory is None:
            return
        os.makedirs(directory, exist_ok=True)
        weight_path = os.path.join(directory, f"CLs_weight{task_id}.pt")
        bias_path = os.path.join(directory, f"CLs_bias{task_id}.pt")
        torch.save(self.fc.weight.detach(), weight_path)
        torch.save(self.fc.bias.detach(), bias_path)
    ...
    def forward(self, x, ortho_loss=False, eval=False):
        if eval:
            try:
                backbone_out = self.backbone(x, eval=True)
            except TypeError:
            ...
            if isinstance(backbone_out, dict):
                ...
                out = self.fc(backbone_out)
                out.update({"features": backbone_out})
            return out

        if self.model_type == 'cnn':
            features = self.backbone(x)
            out = self.fc(features["features"])
            out.update(features)
        else:
            if ortho_loss:
                try:
                    features, extra_loss = self.backbone(x, loss=True)
                except TypeError:
                    features = self.backbone(x)
                    extra_loss = None
